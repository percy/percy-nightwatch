...
async function processFrame(browser, iframe, serializeOptions, domScript, log) {
  try {
    ...
    log?.debug?.(`Successfully captured cross-origin iframe: ${frameResult.frameUrl} (percyElementId: ${iframe.percyElementId})`);
    ...
    try {
      await promisifyBrowserCommand(browser, 'frame', null);
    } catch (e) {
...
async function captureCorsIframes(browser, pageUrl, serializeOptions, domScript, log) {
  try {
    ...
    // Note: only top-level iframes are enumerated. Nested iframes inside
    // same-origin children are not captured (known v1 limitation).
    ...
    if (!iframeInfo || !iframeInfo.length) return [];
    ...
    for (let iframe of iframeInfo) {
      ...
      let result = await processFrame(browser, iframe, serializeOptions, domScript, log);
      if (result) corsIframes.push(result);
    }

    log?.debug?.(`Found ${corsIframes.length + skippedCount} iframe(s): ${corsIframes.length} cross-origin captured, ${skippedCount} skipped`);
