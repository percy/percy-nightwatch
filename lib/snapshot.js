...
// Aliased to the local _FRAME_ names this module already uses.
const {
...
async function processFrameTree(browser, iframe, depth, ancestorUrls, ctx) {
  ...
  try {
    ...
    let iframeElement = await executeScript(browser, function(percyId) {
      ...
      const safeId = (cssApi && cssApi.escape) ? cssApi.escape(percyId) : String(percyId).replace(/["\\]/g, '\\$&');
...
// Backward-compatibility shim for the two trailing positional arguments.
//
// captureDOM is exported and was published (v2.2.1-beta.0) with the signature
//   captureDOM(browser, options, utils, log, domScript)
// This branch reordered the trailing pair to (..., domScript, log) so the
// per-call domScript + logger could be threaded through ctx. Silently swapping
// two published positional args would break any external caller that imports
// captureDOM directly and still passes them in the old order.
//
// domScript is always the PercyDOM source (a string); log is always a logger
// object (or null/undefined). They are never the same type, so we can accept
// either order and route each value to its correct slot. This keeps both the
// old (log, domScript) and new (domScript, log) call shapes working.
function normalizeScriptAndLog(a, b) {
  const isScript = v => typeof v === 'string';
  if (isScript(a) && !isScript(b)) return { domScript: a, log: b ?? null };
  if (isScript(b) && !isScript(a)) return { domScript: b, log: a ?? null };
  // Neither is a string (e.g. both null, or domScript passed as non-string):
  // fall back to the new positional order without reinterpreting.
  return { domScript: a ?? null, log: b ?? null };
}

async function captureDOM(browser, options = {}, utils, arg4 = null, arg5 = null) {
  const { domScript, log } = normalizeScriptAndLog(arg4, arg5);
