...
async function processFrame(browser, iframe, options, domScript, log) {
  try {
...
async function captureCorsIframes(browser, pageUrl, options, domScript, log) {
  try {
    ...
    for (let iframe of iframeInfo) {
      ...
      let result = await processFrame(browser, iframe, options, domScript, log);
...
async function captureSerializedDOM(browser, options = {}, utils, domScript, log) {
  ...
  if (domScript) {
    let corsIframes = await captureCorsIframes(browser, url, options, domScript, log);
