...
// current frame once).
async function processFrameTree(browser, iframe, depth, ancestorUrls, serializeOptions, domScript, log) {
  ...
  try {
    ...
    if (depth < MAX_FRAME_DEPTH) {
      ...
      for (let child of childIframes) {
        ...
        let nested = await processFrameTree(browser, child, depth + 1, nextAncestors, serializeOptions, domScript, log);
    ...
    if (switchedIn) {
      ...
      try {
        ...
        if (depth > 1) {
          ...
          // Carry the partial capture along with the error so the top-level
...
async function captureCorsIframes(browser, pageUrl, serializeOptions, domScript, log) {
  try {
    ...
    for (let iframe of iframeInfo) {
      ...
      let entries;
      try {
        entries = await processFrameTree(browser, iframe, 1, new Set([pageUrl]), serializeOptions, domScript, log);
      } catch (error) {
        if (error && error.percyContextLost) {
