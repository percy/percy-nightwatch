...
// Set once at the top of percySnapshot — domScript (the PercyDOM source
// fetched via @percy/sdk-utils) and log are constant for the duration of a
// snapshot, so we hold them at module scope instead of threading through
...
async function processFrameTree(browser, iframe, depth, ancestorUrls, ctx) {
  const { maxFrameDepth, ignoreSelectors, serializeOptions } = ctx;
...
async function captureSerializedDOM(browser, options = {}, utils) {
  ...
  // Capture cross-origin iframes. domScript is set via setSnapshotContext at
  // the top of the percySnapshot command, so we don't re-check it here.
  const ctx = {
...
async function maybeReloadPage(browser) {
...
async function captureResponsiveDOM(browser, options, utils) {
  ...
  for (let width of responsiveWidths) {
    ...
    const reloaded = await maybeReloadPage(browser);
    if (reloaded) {
      await executeScript(browser, function() {
        /* eslint-disable-next-line no-undef */
        PercyDOM.waitForResize();
...
async function captureDOM(browser, options = {}, utils) {
  const responsiveEnabled = isResponsiveOptionEnabled(options, utils);
  if (responsiveEnabled) {
    return await captureResponsiveDOM(browser, options, utils);
  }
