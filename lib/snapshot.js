...
async function enumerateIframesInCurrentContext(browser) {
  let info = await executeScript(browser, function() {
...
// the parent context on exit. Bounded by MAX_FRAME_DEPTH to prevent runaway
// recursion when pages link to each other in cycles.
async function processFrameTree(browser, iframe, depth, serializeOptions, domScript, log) {
  ...
  try {
    ...
    if (depth < MAX_FRAME_DEPTH) {
      ...
      let childIframes = await enumerateIframesInCurrentContext(browser);
      for (let child of childIframes) {
        if (shouldSkipIframe(child, currentOrigin, log)) continue;
        let nested = await processFrameTree(browser, child, depth + 1, serializeOptions, domScript, log);
    ...
    if (switchedIn) {
      ...
      // own context. Fall back to switching to the top if parentFrame fails.
      try {
...
async function captureCorsIframes(browser, pageUrl, serializeOptions, domScript, log) {
  try {
    let iframeInfo = await enumerateIframesInCurrentContext(browser);
    ...
    for (let iframe of iframeInfo) {
      ...
      let entries = await processFrameTree(browser, iframe, 1, serializeOptions, domScript, log);
...
async function captureSerializedDOM(browser, options = {}, utils, domScript, log) {
  ...
  if (domScript) {
    let corsIframes = await captureCorsIframes(browser, url, serializeOptions, domScript, log);
