...
// Hard ceiling so a misconfigured option can't trigger pathological recursion
...
function resolveMaxFrameDepth(options = {}, utils) {
  const raw = options.maxIframeDepth
...
async function processFrameTree(browser, iframe, depth, ancestorUrls, ctx) {
  ...
  try {
    ...
    if (depth < maxFrameDepth) {
      ...
      let childIframes = await enumerateIframesInCurrentContext(browser, ignoreSelectors);
...
async function captureCorsIframes(browser, pageUrl, ctx) {
  ...
  try {
    let iframeInfo = await enumerateIframesInCurrentContext(browser, ignoreSelectors);
